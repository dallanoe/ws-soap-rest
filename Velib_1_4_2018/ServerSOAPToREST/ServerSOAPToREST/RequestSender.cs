...
namespace ServerSOAPToREST
{
    class RequestSender
    {
        ...
        internal string GetStationDetails(string cityName, string stationName, User user)
        {
            try
            {
                JArray json = JArray.Parse(SendRequestAndGetResponse(cityName));
                ...
                foreach (JObject obj in json)
                {
                    ...
                    {
                        String toReturn = JsonConvert.DeserializeObject<Station>(obj.ToString()).WriteDescription();
                        user.InsertToCache<string>(cityName + stationName, toReturn);
                        return toReturn;
                    }
                }
            }
